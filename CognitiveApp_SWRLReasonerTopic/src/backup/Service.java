	...
	public void postStuff(String rdf, @Context final HttpServletResponse servletResponse, @Context final HttpServletRequest servletRequest, @Context final ServletContext context) throws IOException, SAXException, OWLOntologyCreationException, OWLOntologyStorageException {
		...
		*/
		...
		if (soln==null) 
	        ...
	        model2.read("http://localhost:8080/CognitiveApp4b/files/output/Patient1Triples.owl", "RDF/XML");
	        ...
	        if (solutions.isEmpty()) 
				System.out.println("Could not find any swrl rules file for this topic: \"" + topic + "\"");
	        else {
	        
	        	//specifies the end of the file name
	        	String end = "_new";
				OWLOntology cumulativeOnto = null;
	        	...
	        	for (ListIterator<QuerySolution> it = solutions.listIterator(); it.hasNext();) {
					...
					String outputPath = context.getRealPath("/files/output/") + "\\" + patientName + end + ".owl";
					...
					if (it.hasNext()) {
						cumulativeOnto = reasoner.action(swrlRuleFile, patient, cumulativeOnto, outputPath, false);
					}
					else {
						reasoner.action(swrlRuleFile, patient, cumulativeOnto, outputPath, true);
					}
					
					/* Attention!! We want to execute all applicable rules, 
					 * so need to use the output patient files as input patient files for the next step. 
					 *  Then we should add the functionality to remove the mediate steps
					 */
					/*
					patient = outputPath;
					end = end + "_new";
