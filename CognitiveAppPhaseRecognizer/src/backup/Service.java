...
public class Service {
	...
	public void postStuff(String rdf, @Context final HttpServletResponse servletResponse, @Context final HttpServletRequest servletRequest, @Context final ServletContext context) throws IOException, SAXException, OWLOntologyCreationException, OWLOntologyStorageException {
		...
		*/
		...
		String sparql_prefixes = ServiceHelper_old.getSparqlPrefixesAsString();		
		String sparql_input_pattern = ServiceHelper_old.getSparqlInputPattern();
		...
		QuerySolution soln = ServiceHelper_old.evaluationOfSPARQLQueryAgainstModel(querystring, model);
		...
		if (soln==null) 
			...
			phaseContext = soln.getResource("phaseContextFile").toString(); 
			OPType = soln.getResource("OPType").toString();
			
			//check
			System.out.println("Context: " + phaseContext);
	        ...
	        model2.read("http://localhost:8080/CognitiveApp2/files/output/RudisRule1.owl", "RDF/XML");
	        model2.read("http://localhost:8080/CognitiveApp2/files/output/RudisRule2.owl", "RDF/XML");
	        model2.read("http://localhost:8080/CognitiveApp2/files/output/RudisRule3.owl", "RDF/XML");
	        model2.read("http://localhost:8080/CognitiveApp2/files/output/RudisRule4.owl", "RDF/XML");
	        
	        // construct a query which would select swrl rules files given OPType
	        String querystring2 = ServiceHelper.getSparqlPatternForRulesForOPType(OPTypePrefix, OPTypeName);
	        ...
	        if (solutions.isEmpty()) 
	        	...
	        	for (QuerySolution solnRule : solutions) {
					...
					// finding the name of individuals file 
					String phaseContextName;
					if (phaseContext.contains("/")) {
						// we do not need the extension within a context name
						if (phaseContext.contains(".")) 
							phaseContextName = phaseContext.substring(phaseContext.lastIndexOf("/")+1, phaseContext.indexOf("."));
						else 
							phaseContextName = phaseContext.substring(phaseContext.lastIndexOf("/")+1, phaseContext.length());
					}
					else {
						// we do not need the extension within a context name
						if (phaseContext.contains(".")) 
							phaseContextName = phaseContext.substring(0, phaseContext.indexOf("."));
						else 
							phaseContextName = phaseContext;
					}
					
					//check
					System.out.println("PhaseContextName: " + phaseContextName);
					...
					phaseDetected = reasoner.action(swrlRuleFile, phaseContext, OPTypeName, outputPath);
					if (phaseDetected)
						break;
				}
	        	if (!phaseDetected)
