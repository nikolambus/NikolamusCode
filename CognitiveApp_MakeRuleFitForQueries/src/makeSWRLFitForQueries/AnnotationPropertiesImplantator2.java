	...
	public void action(String swrlRule, String outputPath) throws OWLOntologyCreationException, IOException {
	    ...
	    while ((line = br.readLine()) != null) {
			...
			if (Pattern.matches("        <rdf:type rdf:resource=\"http://www.w3.org/2003/11/swrl#Imp\"/>", line)) {
				...
				writer.println("        <base:Property-3AHas_name rdf:resource=\"http://localhost/mediawiki/index.php/Special:URIResolver/"+ruleName+"\"/>");
				writer.println("        <base:Property-3AHas_source_rule rdf:resource=\""+swrlRule+"\"/>");
				for (int i=0; i < classNames.size(); i++)
					writer.println("        <base:Property-3AHas_topic rdf:resource=\"" + base + classNames.get(i) + "\"/>");
			}
			...
			if (Pattern.matches("\\s*<rdf:type rdf:resource=\"http://www.w3.org/2003/11/swrl#ClassAtom\"/>\\s*", line)) {
				...
				*/
				...
				if ((bodyFlag) && (!conclusionClassesNames.contains(atomClsName))) {
					...
					// body --> premise
					writer.println(tab + "<base:Property-3AIs_premise_of rdf:resource=\"http://localhost/mediawiki/index.php/Special:URIResolver/" + currentRuleName + "\"/>");	
				}
				else {
				
					// head --> conclusion
					writer.println(tab + "<base:Property-3AIs_conclusion_of rdf:resource=\"http://localhost/mediawiki/index.php/Special:URIResolver/" + currentRuleName + "\"/>");	
				}
